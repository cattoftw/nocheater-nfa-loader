...
/// Read + decrypt the ConnectCache JWT for `account_name` from local.vdf (if present).
pub(crate) fn read_connect_cache_token(account_name: &str) -> Option<String> {
    if account_name.trim().is_empty() {
        return None;
    }
    let hex = read_connect_cache_blob(account_name)?;
    steam_decrypt(&hex, account_name)
        .ok()
        .filter(|t| !t.trim().is_empty())
}

fn read_connect_cache_blob(account_name: &str) -> Option<String> {
    if account_name.trim().is_empty() {
        return None;
    }
    let base = local_steam_cache_path().ok()?;
    let path = base.join("local.vdf");
    let content = fs::read_to_string(path).ok()?;
    let crc = compute_crc32(account_name);
    find_connect_cache_blob(&content, &crc)
}

fn find_connect_cache_blob(content: &str, crc: &str) -> Option<String> {
